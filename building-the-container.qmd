...
`knitr::purl()` takes as an argument `analysis.qmd` to outputs `analysis.R`. The main reason I chose to do this is because keeping all your code in one place leads to fewer errors and it is easier to maintain. The argument `documentation` controls how much of the prose is ported as comments into the `.R` file. I chose the value of `1` for just the code. 
...
### When to Consider Packaging Your Analysis:
...
1. **Consistency**: Containers ensure that your analysis runs the same way on any environment, eliminating the "it works on my machine" problem.
2. **Reproducibility**: By packaging your analysis in a container, you capture the exact environment, including dependencies and configurations, making it easier for others to reproduce your results.
3. **Ease of Collaboration**: Sharing a container is straightforward, allowing collaborators to run your analysis without worrying about setting up their environment.
4. **Simplified Deployment**: Containers simplify the process of deploying your analysis to different platforms, whether it's a local machine, a cloud service, or a shared research cluster.
...
To build a container we are going to create a file called Dockerfile which have the instructions needed to build a container image. (Think of the container image as the instructions).
...
readr::write_lines(WORKDIR_line, file = "Dockerfile", append = TRUE)
readr::write_lines(COPY_renv_lock, file = "Dockerfile", append = TRUE)
readr::write_lines(COPY_renv_library, file = "Dockerfile", append = TRUE)
...
readr::write_lines(read_lines("install_and_restore_packages.txt"), file = "Dockerfile", append = TRUE)

```

## Build the image 

Once we have a Dockerfile, an image is created by running a `podman image build` command. I chose `podman` rather than `docker` due to recent changes to the Docker License Agreement that can potentially affect UW-Madison users. Read more about this issue [here](https://bcrf.biochem.wisc.edu/2023/11/17/install-local-docker-desktop-on-macos-without-admin/#:~:text=Important%20note%20(oct%2007%2C%202024,Hero%20%7C%20Full%201%20Hour%20Course).
...
As your image was already created, the next step is to push the image to the GitLab Registry. The code chunk below does just that. 
...
- You will need to copy over to CHTC `analysis.R` and `data.csv`. There is a code chunk that will accomplish that below. 
- You will also need to copy over two additional files, a submit file and a executable file. There is a code chunk below that will create a copy those files over to CHTC.  

### Copying your analysis and data to CHTC 

The Linux command `scp` — secure copy — can take care of copying files from one machine to another. A popup window will ask you to authenticate with your NetID, password, and your MFA device. Follow the prompts. The first chunk will copy the `analysis.R` file; the second chunk will copy over the `data.cvs` file 

```{bash}

#scp analysis.R lares@ap2001.chtc.wisc.edu:/home/lares

```

```{bash}

#scp data.csv lares@ap2001.chtc.wisc.edu:/home/lares
...
# The “transfer files” lines all tell HTCondor 
#what to do with output and input files
transfer_input_files = analysis.R, data.csv
transfer_output_files = results-folder

#The log, output and error files all track 
#information printed by the job or about the job
log = job.log
error = job.err
output = job.out

# The request stanza tells HTCondor how many 
#resources on the computer that you need to run 
#your jobs.
request_cpus = 1
request_memory = 4GB
request_disk = 2GB

# The queue stanza creates the number of separate jobs indicated when the submit file is submitted. 

queue 1

```

This code chunk copies the content of `analysis.sub` over to CHTC. 

```{bash}

#scp analysis.sub lares@ap2001.chtc.wisc.edu:/home/lares
...
This code chunk copies the `analysis.sh` over to CHTC

```{bash}

#scp analysis.sh lares@ap2001.chtc.wisc.edu:/home/lares
...
If you want to track the progress, you can run a `condor_watch_q` command. You can read more about it [here](https://chtc.cs.wisc.edu/uw-research-computing/condor_q)
